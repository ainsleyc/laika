...
function App(config) {
  ...
  this.start = function start(dbname, port, settingsFile) {
    ...
    process.env.MONGO_URL = App.getMongoUrl(config.mongoHost, config.mongoPort, this.dbname);
  ...
  this.close = function close(callback) {
    ...
    if(this.meteorApp) {
      ...
      var mongoUrl = App.getMongoUrl(config.mongoHost, config.mongoPort, this.dbname);
...
App.getMongoUrl = function getMongoUrl(host, port, dbname) {
...
App.touch = function touch(callback, options) {
  ...
  var dbname = '_laika';
  var dbUrl = App.getMongoUrl(options.mongoHost, options.mongoPort, dbname);
  ...
  function onData(data) {
    if(data.toString().match('http://localhost:')) {
      callback();
      cleanup();
    }
